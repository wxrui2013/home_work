...
integer:: timestep,natoms,i,j,k,k2,l,l2,m,x,filev,atoms_number1,atoms_number2,atoms_number3,other,tmp,tmp2,tmp3,tmp4,tmp_x,N
integer:: config,line_number,bonds_number,counter,min_cluster,min_c_id,min_c,max_c_id,max_c,c_id,triang_number
integer:: c_number(2500),isolate,chain,cluster_tri,joint
...
integer,parameter ::n_col =34						!34 do not change normally
!column number for record information about ico_cluster :id,c1..c30,cluster_id,state.bonds_number


type(atoms),allocatable::atom(:)			!atom structure information
type(atomcn),allocatable::cn(:)				!corrdination number
integer ,allocatable ::atom_status(:)		!atom state for output some atom
integer,allocatable ::ico_id(:)				!id number of icosehadral
integer,allocatable ::index_id(:)			!index array about structure
...
open (unit=6,file="dis_cluster0.dat")
...
open (unit=26,file="cluster_n_s.xyz")			!one type cluster that have n center icosehedra
open (unit=28,file="cluster_coeff0.txt")  
...
!do filev=1,51				!----average of all configurations-------
...
enddo
write (*,*) "reading all done!","configuration=" ,n_con+1
...
!------------------------array method to achieve cluster-------------------------
...
do i=1,line_number
	...
	cluster_coeff(i)=real(2*ico_full_id(i,34))/real(ico_full_id(i,33)*(ico_full_id(i,33)-1)) 	!.X=2L/N_bonds*(N_bonds-1)
	...
	if (ico_full_id(i,33) == 0 ) then
		isolate=isolate+1
	elseif(ico_full_id(i,33) == 1) then
		chain=chain+1
	elseif(ico_full_id(i,33) == 2) then
		if(ico_full_id(i,34) /= 0 ) then
			cluster_tri=cluster_tri+1
		else
			chain=chain+1
		endif
	elseif(ico_full_id(i,33) >=3 ) then
		if (ico_full_id(i,34) >= 1) then
			cluster_tri=cluster_tri+1
		else
			joint=joint+1
		endif
...
write(28,*)	"cluster_number cluster_coefficient isolate chain cluster joint"
write(28,102) line_number,tmp_real/real(line_number),real(isolate)/real(line_number),real(chain)/real(line_number) &
			& ,real(cluster_tri)/real(line_number),real(joint)/real(line_number)
102		format(i7,1X,f6.5,1X,4(f6.5,1X))
...
!-------------------------------------------------------------------------------
!---------output one inter-ico-cluster---------------------------
atom_status(:)=0
	k=4		!test point!!!!!		!atoms number of cluster (k<=max_cluster_id)  cluster_id
	counter=0						!
do i=1,line_number
